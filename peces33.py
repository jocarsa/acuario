...
def make_vertical_gradient(h, w, top_color, bottom_color):
    """BGR top->bottom vertical gradient."""
    ...
    grad = (1.0 - t) * top + t * bottom                   # (h,3)
    grad = np.repeat(grad[None, :, :], w, axis=0).transpose(1,0,2)  # (h,w,3)
    return grad.astype(np.uint8)

# Precompute background gradient (light blue -> deep blue)
BG_GRADIENT = make_vertical_gradient(
    height, width,
    top_color=(255, 235, 200),   # light bluish (B,G,R)
    bottom_color=(60, 40, 10)    # deep blue-ish
)
...
class Comida:
    def __init__(self, x=None, y=None, radius=None, angle=None, v=None, layer=None):
        ...
        self.layer = layer if layer is not None else random.choices([0,1,2], weights=[0.4,0.35,0.25])[0]  # 0=back,1=mid,2=front
        base_r = radius if radius is not None else random.uniform(5, 15)
        # Slight size scaling by depth (farther = smaller)
...
# =========================
# Fish with memory & genetics + wall avoidance + depth layers
# =========================
...
class Pez:
    def __init__(self, parent=None):
        ...
        if parent is None:
            ...
            # Inherit layer with small chance to change
    ...
    def mueve(self, quadtree, frame_count):
        ...
        thrust = self.base_max_thrust * max(math.sin(2*math.pi*self.flapping_phase), 0)
        # NOTE: thrust already scaled via flapping_frequency/max_thrust above if you prefer; keeping original feel
        drag = self.drag_coefficient * self.speed
        self.speed += (max_thrust if 'max_thrust' in locals() else self.base_max_thrust) * max(math.sin(2*math.pi*self.flapping_phase), 0) - drag
    ...
    def maybe_reproduce(self, fishes, frame_count, max_pop):
        ...
        if self.energia > breed_threshold:
            ...
            # Child keeps (mostly) same layer in __init__
            fishes.append(child)
...
for frame_count in range(total_frames):
    # Create three separate drawing layers (BGR)
    ...
    # Evolve food (draw on its depth layer)
    for comida in comidas:
        comida.vive(LAYERS[comida.layer])
    ...
    # Update fish, remove dead (draw on their depth layer)
    for pez in peces[:]:
        pez.vive(LAYERS[pez.layer], fish_qt, frame_count)
        if pez.energia <= 0:
            peces.remove(pez)
    ...
    # ===== Compose final frame with depth-of-field effect =====
    # Start from gradient background
    frame = BG_GRADIENT.copy()

    # Blur background & mid layers (strong / light)
    back_blur = cv2.GaussianBlur(layer_back, (31, 31), 0)
    mid_blur  = cv2.GaussianBlur(layer_mid,  (11, 11), 0)

    # Additively blend layers over the gradient (simple, pleasing result)
    frame = cv2.add(frame, back_blur)
    frame = cv2.add(frame, mid_blur)
    frame = cv2.add(frame, layer_front)
