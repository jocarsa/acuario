...
class Pez:
    ...
    def mueve(self):
        ...
        if self.is_stuck:
            ...
            center_x = sum(fish.x for fish in peces if math.hypot(self.x - fish.x, self.y - fish.y) < repulsion_radius) / nearby_fish_count
            center_y = sum(fish.y for fish in peces if math.hypot(self.x - fish.x, self.y - fish.y) < repulsion_radius) / nearby_fish_count
